"""
RF Sensor Protocol Implementation

This module provides a Python interface for communicating with RF sensor devices
using a custom ASCII-based frame protocol over serial port.
...
- Deep integration with AsyncSerialPort FRAME mode
- Automatic checksum calculation and validation
- Command/response matching
- Hybrid error handling (exceptions + return values)
...
class RFSensorProtocol:
    """
    RF Sensor communication class with deep AsyncSerialPort integration.
    
    This class provides a high-level interface for RF sensor commands while
    leveraging AsyncSerialPort's FRAME mode for robust serial communication.
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to AsyncSerialPort
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize RF sensor protocol handler"""
        # Setup logging
        self._logger = logging.getLogger(f"RFSensor({port})")
        if debug:
        ...
        if response_parser is None:
            print("DEBUG: RFSensorProtocol: Using default response parser")
        
        # Initialize AsyncSerialPort with FRAME mode
        # Note: RF protocol has variable-length frames, so we use a custom approach
        self._serial = AsyncSerialPort(
            ...
            frame_length=24,  # Minimum frame length (will be overridden by parser)
        ...
        self._logger.info(f"RFSensorProtocol initialized on {port}")
    ...
    def build_set_command(command_byte: bytes, data_value: Any) -> bytes:
        ...
        Example:
            >>> RFSensorProtocol.build_set_command(b'M', 2)
            b'<M=2BC>'
        """
    ...
    def build_read_command(command_byte: bytes) -> bytes:
        ...
        Example:
            >>> RFSensorProtocol.build_read_command(b'M')
            b'<M4D>'
        """
    ...
    def parse_response(response_bytes: bytes) -> Dict[str, Any]:
        ...
        Example:
            >>> RFSensorProtocol.parse_response(b'[M=2BC]')
            {'command': 'M', 'data': '2', 'raw': b'[M=2BC]'}
        """
        print(f"DEBUG: parse_response: response_bytes={response_bytes}")
    ...
    def _create_response_parser(self) -> 'Callable[[bytes], Tuple[Optional[str], bytes]]':
        """
        Create custom response parser for AsyncSerialPort integration.

        This parser extracts command_id from RF protocol responses to enable
        command-response matching in AsyncSerialPort.

        Returns:
            Callable[[bytes], Tuple[Optional[str], bytes]]: parser function that takes response bytes and returns a tuple of (command_id, response_data)
        """
        self._logger.debug(f"DEBUG: _create_response_parser")
        def rf_response_parser(response: bytes) -> Tuple[Optional[str], bytes]:
            ...
            """
            try:
                self._logger.debug(f"DEBUG: rf_response_parser: response={response}")
                # Parse the response
    ...
    async def connect_async(self) -> bool:
        """
        Asynchronously connect to RF sensor.
        ...
        """
        return await self._serial.connect_async()
    
    def connect(self, timeout: float = 10.0) -> bool:
        """
        Synchronously connect to RF sensor.
        ...
        Raises:
            ConnectionError: If connection fails
        """
        return self._serial.connect(timeout=timeout)
    
    async def disconnect_async(self):
        """Asynchronously disconnect from RF sensor"""
        await self._serial.disconnect_async()
    
    def disconnect(self, timeout: float = 5.0):
        """
        Synchronously disconnect from RF sensor.
        
        Args:
            timeout: Disconnection timeout in seconds
        """
        self._serial.disconnect(timeout=timeout)
    
    @property
    def is_connected(self) -> bool:
        """Check if connected to RF sensor"""
        return self._serial.is_connected
    
    # ==================== Low-Level Command Interface ====================
    
    async def send_rf_command_async(
        self,
        command_byte: bytes,
        ...
        """
        Send RF command and wait for response (async).
        ...
        response = await self._serial.send_command_async(
        ...
        print(f"DEBUG: send_rf_command_async response={response}")
    ...
    def send_rf_command(
        ...
        """
        Send RF command and wait for response (sync).
        ...
        Raises:
            ...
            FrameError: Invalid response format
        """
        return self._serial._loop.run_until_complete(
            self.send_rf_command_async(command_byte, data_value, timeout)
        )
    ...
    async def set_communication_medium_async(
        self,
        medium: CommunicationMedium,
        timeout: Optional[float] = None
    ) -> bool:
        """
        Set communication medium (async).
        ...
        response = await self.send_rf_command_async(
    ...
    def set_communication_medium(
        ...
        """
        return self._serial._loop.run_until_complete(
            self.set_communication_medium_async(medium, timeout)
        )
    
    async def get_communication_medium_async(
        self,
        timeout: Optional[float] = None
    ) -> Optional[CommunicationMedium]:
        """
        Get current communication medium (async).
    ...
    def get_communication_medium(
        ...
        """
        Get current communication medium (sync).
        ...
        """
        return self._serial._loop.run_until_complete(
            self.get_communication_medium_async(timeout)
        )
    
    # --- Channel ---
    
    async def set_channel_async(
        self,
        channel: int,
        timeout: Optional[float] = None
    ) -> bool:
        """
        Set communication channel (async).
    ...
    def set_channel(
        ...
        """
        Set communication channel (sync).
        ...
        """
        return self._serial._loop.run_until_complete(
            self.set_channel_async(channel, timeout)
        )
    ...
    async def get_channel_async(
        ...
        """
        Get current communication channel (async).
    ...
    def get_channel(
        ...
        """
        Get current communication channel (sync).
        ...
        """
        return self._serial._loop.run_until_complete(
            self.get_channel_async(timeout)
        )
    
    # --- Device ID ---
    
    async def set_device_id_async(
        self,
        device_id: Any,
        timeout: Optional[float] = None
    ) -> bool:
    ...
    def set_device_id(
        ...
        Args:
            device_id: 6-digit ID (string or int)
            timeout: Response timeout
        ...
        """
        return self._serial._loop.run_until_complete(
            self.set_device_id_async(device_id, timeout)
        )
    
    async def get_device_id_async(
        self,
        timeout: Optional[float] = None
    ) -> Optional[str]:
        """
        Get current device ID (async).
    ...
    def get_device_id(
        ...
        """
        return self._serial._loop.run_until_complete(
            self.get_device_id_async(timeout)
        )
    
    # --- Port ---
    
    async def set_port_async(
        self,
        port: int,
        timeout: Optional[float] = None
    ) -> bool:
        """
        Set port setting (async).
    ...
    def set_port(
        ...
        """
        return self._serial._loop.run_until_complete(
            self.set_port_async(port, timeout)
        )
    
    async def get_port_async(
        self,
        timeout: Optional[float] = None
    ) -> Optional[int]:
        """
        Get current port setting (async).
    ...
    def get_port(
        ...
        """
        return self._serial._loop.run_until_complete(
            self.get_port_async(timeout)
        )
    
    # --- RF Status ---
    
    async def read_rf_status_async(
        self,
        timeout: Optional[float] = None
    ) -> bool:
        """
        Read RF configuration status (async).
    ...
    def read_rf_status(
        ...
        """
        Read RF configuration status (sync).
        
        Args:
            timeout: Response timeout
            
        Returns:
            True if RF configuration OK, False if failed
        """
        return self._serial._loop.run_until_complete(
            self.read_rf_status_async(timeout)
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        """
        return self._serial.statistics
    ...
    def get_response_time_stats(self) -> Dict[str, Any]:
        ...
        """
        return self._serial.get_response_time_stats()
    ...
    def __repr__(self):
        return (
            f"RFSensorProtocol(port={self._port}, "
...
def example_sync_usage():
    ...
    try:
        with RFSensorProtocol(COM_PORT, timeout=2.0, debug=True) as sensor:
            ...
            if sensor.set_device_id("000120"):
                print("✓ Device ID set\n")
            else:
...
async def example_async_usage():
    ...
    print("\n" + "="*60)
    print("RF Sensor - Asynchronous Usage Example")
    print("="*60 + "\n")
    ...
    # Initialize and configure device
    try:
        async with RFSensorProtocol(COM_PORT, timeout=5.0, debug=True) as sensor:
            print("✓ Connected to RF sensor\n")
            
            # Set communication to 5G
            print("Setting communication medium to 5G...")
            if await sensor.set_communication_medium_async(CommunicationMedium.WIFI_5G):
                print("✓ Medium set to 5G\n")
            
            # Set channel to 128
            print("Setting channel to 128...")
            if await sensor.set_channel_async(128):
                print("✓ Channel set to 128\n")
            
            # Set device ID
            print("Setting device ID to 000120...")
            if await sensor.set_device_id_async("000120"):
                print("✓ Device ID set\n")
            
            # Set port
            print("Setting port to 2...")
            if await sensor.set_port_async(2):
                print("✓ Port set to 2\n")
...
def test_checksum_calculation():
    ...
    for data, expected, description in test_cases:
        ...
        print(f"  Data:     {data}")
        print(f"  Expected: {expected.decode()}")
        print(f"  Got:      {result.decode()}")
...
def test_frame_construction():
    ...
    for command, data, expected, description in test_cases:
        result = RFSensorProtocol.build_set_command(command, data)
        status = "✓ PASS" if result == expected else "✗ FAIL"
        print(f"{status} - {description}")
    ...
    for command, expected, description in read_cases:
        result = RFSensorProtocol.build_read_command(command)
        status = "✓ PASS" if result == expected else "✗ FAIL"
        print(f"{status} - {description}")
        print(f"  Expected: {expected}")
        print(f"  Got:      {result}")
...
def test_frame_parsing():
    ...
    for frame, exp_cmd, exp_data, description in test_cases:
        try:
            result = RFSensorProtocol.parse_response(frame)
...
def test_invalid_frames():
    ...
    for frame, expected_exception, description in test_cases:
        try:
            RFSensorProtocol.parse_response(frame)
            print(f"✗ FAIL - {description}")
            print(f"  Expected exception: {expected_exception.__name__}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "test":
        ...
        elif command == "example-sync":
            # Run synchronous example
            example_sync_usage()
        
        elif command == "example-async":
            ...
            print("\nUsage:")
            print("  python RF_sensor.py test          - Run unit tests")
            print("  python RF_sensor.py example-sync  - Run sync example")
            print("  python RF_sensor.py example-async - Run async example")
