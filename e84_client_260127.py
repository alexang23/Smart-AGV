...
class E84Client(AsyncSerialPort):
    ...
    #     """
    #     Ensure the event queue is created in the running event loop.
    #     """
    #     await super().connect_async(*args, **kwargs)
    #     print(f"5555555555555555555555555 DEBUG: Before E84Client connect_async")
    #     if self._event_queue_size and self._event_queue is None:
    #         self._event_queue = asyncio.Queue(maxsize=self._event_queue_size)
    #         self.logger.info(f"事件佇列建立: size={self._event_queue_size}")
    #     print(f"5555555555555555555555555 DEBUG: After E84Client connect_async")
    ...
    async def _handle_sensor_event(self, message: E84Message) -> None:
        ...
        if self._event_queue is None and getattr(self, "_event_queue_size", None):
            self._event_queue = asyncio.Queue(maxsize=self._event_queue_size)
            self.logger.info(f"事件佇列延後建立: size={self._event_queue_size}")
    ...
    async def db25_port_control(self, open: bool = True) -> bool:
        ...
        if response:
            self.logger.info(f"------------------ DB25 Port {'打開' if open else '關閉'}")
            return True
        return False
